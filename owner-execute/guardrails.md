...
执行场景的共同护栏。回答"什么不能破坏"，不是替代执行流程。

先按目标粒度走 Task 执行或 Roadmap 推进，再用这里做自检。不要把本文件改造成每轮审计报告。
...
- 当前/最新阶段必须列出一批 planned 条目，让 AI 下一轮能直接开工。
...
- 产品/领域洞见 → `.boss/wiki/`
- 架构决策 → `.boss/architecture/`（owner-meeting 落架构）
- 任务执行细节 → `.boss/tasks/` 或 `.boss/tasks/_resolved/`

## Requirements 护栏

requirements 是需求锚点，不是计划、任务或实现方案的容器。详细写作约束在 owner-meeting 的「落需求」段。

如果用户需求变了，先回 owner-meeting 更新 requirements，再更新 Roadmap/tasks。
...
task 是有规划、有思想、有深度的工作单元。禁止把原子化编辑伪装成独立 task：

- **禁止原子 task**：一个 task 至少包含多个关联步骤，交付可感知的工程或产品价值。把单个函数提取、单次文件拆分、单个测试新增当作一个独立 task 属于抽象层级错误。
- **同类合并**：同一目标下的同类小改动（如同一模块的多次拆分、同一功能边界的多个边界覆盖、同一轮验收的多个检查点）必须合并在一个 task 内。
- **执行方案多步骤**：task 的执行方案不能只写一步。如果执行方案只有一步，说明体量不够，应该合并。
- **判断标准**：回头看本 task 的 diff——如果改动文件数和行数太少、能用一句话不加"和"字描述完，说明太碎。如果 task 文档比代码改动还长，说明太碎。
- **commit 不是 task 边界**：一个 task 可以包含一个或多个 commit。commit 是交付载体，task 是工作单元——不要 1:1 映射。
...
完整实践指南见 [工作区清理](guides/worktree-cleanup.md)。每轮结束时工作区应该尽量回到干净；不能干净时，剩余文件必须有归属、有原因、有下一步。不要把脏工作区遗留给下一轮猜。
...
实战发现的问题，值得修就建 task 或直接修；属于新需求就回到 owner-meeting 讨论；纯观察就记入 wiki 或 task 记录。
