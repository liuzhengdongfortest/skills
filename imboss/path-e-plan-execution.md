...
新建 plan 或推进到新阶段时，当前/最新阶段必须先列出一批初步待执行条目。条目可以粗，但必须足够让 Agent CLI 下一轮直接选择一个开工；不能只写“阶段 N：待拆”或只留下阶段标题。
...
如果主 plan 没有当前阶段待执行条目，先在 plan 内拆条目，不要预建 task 文档。拆完条目后继续执行，不要只停在“已补计划”。
...
当前/最新阶段的条目必须在列 plan 时就初步写出来。最低要求：

- 至少 3 个条目，除非阶段天然少于 3 个动作
- 每个条目是一句话，能被执行者直接选择开工
- 每个条目有状态摘要：`planned`、`active`、`blocked` 或 `done`
- 条目不能展开目标、范围、验收、验证；这些开工时进 task 文档
...
- 禁止只汇报当前状态后询问“是否开工”
- 禁止问“要不要继续”“是否推进下一个 task”
- 禁止把计划条目复述一遍后停在等待确认
- 禁止把“我可以做 X”当成本轮交付

正确动作是：读取当前状态，选择下一个安全、可验证、需求锚定的 plan 条目；若条目还没有 task 文档，先创建 task 文档；然后执行、验证、更新 task/plan/交付物。

只有老板明确要求暂停/停下/先不要做，或者没有任何安全、可验证、需求锚定的工作可做，才允许停下来。需要老板拍板的点也不应阻塞整体进程：记录阻塞，切换到同一目标下其他安全工作。

如果老板只是问“现在怎么样”，可以先回答状态；但除非老板明确要求只汇报或暂停，回答后仍应继续推进。
