...
hook prompt 不应该复制整套执行规程，而应该唤起 boss 技能里的持续工作模式。执行 AI 进入 boss 技能后，会读 [`../guides/continuous-work.md`](../guides/continuous-work.md)，规程入口动作是先做一次大抬头（见 [`../guides/lookup-beats.md`](../guides/lookup-beats.md)）。
...
```text
Use the boss skill. Follow its 持续工作模式执行规程 before stopping.
...
- 具体长期状态。
- 可以从 `.boss/` 读取的事实。
- 持续工作模式执行规程全文；执行 AI 应从 boss 技能文件读取。
