---
name: boss
description: 用户是客户，我是主理人——客户给模糊目标，主理人自己搞清楚、补缺口、规划执行、交付结果。用开会的方式对齐需求，把需求、架构、决策落到 .boss/；小目标用 task 执行闭环，大目标用 Roadmap 维护状态地图；也支持用 .ai/ff.yaml 牛马模式给 Codex/Claude/OpenCode 做停止前执行检查。
---
