...
Task 执行用于小目标：目标清楚、范围有限、能在一次执行闭环里完成和验证。
...
4. 写执行方案：步骤、边界、验证、风险。
5. 执行改动；新增业务逻辑按 [工程治理](../guides/engineering-governance.md) 判断是否新建模块。
6. 执行中按 [执行中发现问题](../guides/execution-discovery.md) 处理新发现。
7. 验证：测试、构建、截图、真实流程、人工检查，按任务风险选择。
8. 更新 task：验证证据、发现的问题、Git 状态、执行记录。
9. 提交小 commit；不能提交时记录原因。
10. 按 [工作区清理](../guides/worktree-cleanup.md) 收尾。
11. 完成后移动 task 到 `.boss/tasks/_resolved/YYYY-MM/`，更新 `.boss/tasks/INDEX.md`。
